...
def _logistic_case(features_path:Path,targets_path:Path,positive_hint=None,max_cols=60):
    ...
    baseline_p=np.full(len(yt),prevalence)
    centroids={c:Z[train][y[train]==c].mean(0) for c in (0,1) if (y[train]==c).any()}
    if len(centroids)==2:
        d0=np.linalg.norm(Z[test]-centroids[0],axis=1)+1e-9; d1=np.linalg.norm(Z[test]-centroids[1],axis=1)+1e-9
        centroid_p=d0/(d0+d1)
    else: centroid_p=np.full(len(yt),prevalence)
    def _comparison_row(name,p):
        cls=(p>=.5).astype(int)
        tpr_c=float(((cls==1)&(yt==1)).sum()/max(1,(yt==1).sum())); tnr_c=float(((cls==0)&(yt==0)).sum()/max(1,(yt==0).sum()))
        return {'model':name,'brier':float(np.mean((p-yt)**2)),'auc':_auc(yt,p),'balanced_accuracy':.5*(tpr_c+tnr_c)}
    comparisons=[_comparison_row('constant_prevalence_baseline',baseline_p),_comparison_row('nearest_centroid_challenger',centroid_p),{'model':'class_balanced_logistic_champion','brier':brier,'auc':auc,'balanced_accuracy':bal}]
    champion=max(comparisons,key=lambda row:((row['auc'] if row['auc'] is not None else -1),row['balanced_accuracy']))['model']
    return {'status':'MODEL_ACTIVE','target':target_name,'rows':n,'fail_or_positive_rate':float(y.mean()),'selected_features':kept[:12],
            ...
            'model_comparison':comparisons,'selected_model':champion,'selection_metric':'highest holdout ROC AUC; balanced accuracy tie-break',
